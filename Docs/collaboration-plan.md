...
- 本次新增 CloudBoards 復原草稿、帳號清單競態、分享 URL 清理及待同步 undo/redo 回歸案例；`npm test` 56 項通過，`npm run test:browser` 通過。瀏覽器案例涵蓋圖層鎖定中止文字／樹節點編輯與拖曳、鍵盤焦點保護、pointer capture 取消及 Mermaid 重載保留尺寸；前端 JS 語法與 `git diff --check` 通過。雲端尚未配置，因此沒有驗證雙帳號、Worker/D1 或正式環境權限流程；設定環境前仍需跑上述負向測試。
